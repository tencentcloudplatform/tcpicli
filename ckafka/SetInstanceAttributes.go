// 2018-01-30 09:51:57.598933 -0800 PST m=+10.392569584
// Code generated by go generate; DO NOT EDIT.
// This file was generated by robots

package ckafka

import (
	"encoding/json"
	"github.com/tencentcloudplatform/tcpicli/core"
)

type SetInstanceAttributesResp struct {
	Code     int64  `json:"code"`
	CodeDesc string `json:"codeDesc"`
	Message  string `json:"message"`
}

// Implement https://cloud.tencent.com/document/api/597/10095
func (c *CkafkaClient) SetInstanceAttributes(options ...string) (*SetInstanceAttributesResp, error) {
	resp, err := c.DoAction("SetInstanceAttributes", options...)
	if err != nil {
		return nil, err
	}
	var s SetInstanceAttributesResp
	err = json.Unmarshal(resp, &s)
	return &s, err
}

func SetInstanceAttributes(options ...string) (*SetInstanceAttributesResp, error) {
	return DefaultClient.SetInstanceAttributes(options...)
}

func (r *SetInstanceAttributesResp) String(args ...interface{}) (string, error) {
	var b []byte
	var err error
	if len(args) == 0 {
		b, err = json.MarshalIndent(r, "", "  ")
	} else if len(args) == 1 {
		if val, ok := args[0].(string); ok {
			if len(val) == 0 {
				b, err = json.MarshalIndent(r, "", "  ")
			} else {
				b, err = core.FmtOutput(val, r)
			}
		}
	}
	return string(b), err
}
