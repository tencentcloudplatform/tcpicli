// 2018-01-08 14:25:08.145854 -0800 PST m=+9.199574556
// Code generated by go generate; DO NOT EDIT.
// This file was generated by robots

package dfw

import (
	"encoding/json"
	"github.com/tencentcloudplatform/tcpicli/core"
)

type ModifySecurityGroupPolicysResp struct {
	Code     int64  `json:"code"`
	CodeDesc string `json:"codeDesc"`
	Message  string `json:"message"`
}

// Implement https://cloud.tencent.com/document/api/213/1365
func (c *DfwClient) ModifySecurityGroupPolicys(options ...string) (*ModifySecurityGroupPolicysResp, error) {
	resp, err := c.DoAction("ModifySecurityGroupPolicys", options...)
	if err != nil {
		return nil, err
	}
	var s ModifySecurityGroupPolicysResp
	err = json.Unmarshal(resp, &s)
	return &s, err
}

func ModifySecurityGroupPolicys(options ...string) (*ModifySecurityGroupPolicysResp, error) {
	return DefaultClient.ModifySecurityGroupPolicys(options...)
}

func (r *ModifySecurityGroupPolicysResp) String(args ...interface{}) (string, error) {
	var b []byte
	var err error
	if len(args) == 0 {
		b, err = json.MarshalIndent(r, "", "  ")
	} else if len(args) == 1 {
		if val, ok := args[0].(string); ok {
			if len(val) == 0 {
				b, err = json.MarshalIndent(r, "", "  ")
			} else {
				b, err = core.FmtOutput(val, r)
			}
		}
	}
	return string(b), err
}
